...
obj = Object(size=Vector3(1,1,1),rotation=Vector3(0,0,0)).add_component([BoxCollider(),Rigidbody(useGravity=False)])
...
camera.add_component(Client("192.168.1.14", 5000, data_objects=[camera],user_name="yaly"))
# camera.add_component(Server(data_objects=[camera]))

scene = [floor]

Core.run(scene+[camera])