...
class CamController(Component):
    def __init__(self,speed=10,speed2=10):
        super(CamController, self).__init__()
    ...
    def keyboard_controller(self, dt):
        ...
        if keyboard.is_pressed('w'):
            forward = self.parent.transform.quaternion.rotate(Vector3(0, 0, 1))
            # flatten to XZ plane
            forward = Vector3(forward.x, 0, forward.z).normalized() * self.force_amount
            self.parent.transform.position += forward * dt

        if keyboard.is_pressed('s'):
            backward = self.parent.transform.quaternion.rotate(Vector3(0, 0, -1))
            backward = Vector3(backward.x, 0, backward.z).normalized() * self.force_amount
            self.parent.transform.position += backward * dt

        if keyboard.is_pressed('a'):
            right = self.parent.transform.quaternion.rotate(Vector3(1, 0, 0))
            right = Vector3(right.x, 0, right.z).normalized() * self.force_amount
            self.parent.transform.position += right * dt

        if keyboard.is_pressed('d'):
            left = self.parent.transform.quaternion.rotate(Vector3(-1, 0, 0))
            left = Vector3(left.x, 0, left.z).normalized() * self.force_amount
            self.parent.transform.position += left * dt

        if keyboard.is_pressed('space'):
            self.parent.transform.position += Vector3(0, self.force_amount2, 0) * dt
